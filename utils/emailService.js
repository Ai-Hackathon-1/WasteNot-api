...
dotenv.config();
// Ensure environment variables are loaded (you must import dotenv/config at app entry)
const EMAIL_FROM = process.env.EMAIL_FROM; // e.g. youraddress@gmail.com
const GMAIL_APP_PASSWORD = process.env.GMAIL_APP_PASSWORD; // Gmail App Password (not your main password)
const EMAIL_FROM_NAME = process.env.EMAIL_FROM_NAME || 'WasteNot App';
const CLIENT_URL = process.env.CLIENT_URL;

if (!EMAIL_FROM || !GMAIL_APP_PASSWORD) {
  console.error('❌ EMAIL_FROM and GMAIL_APP_PASSWORD must be set in your environment variables.');
}
...
class EmailService {
  ...
  createTransport() {
    ...
    return nodemailer.createTransport({
      ...
      // Remove pool/maxConnections for simplicity. Add back if needed.
    });
  }
  ...
  async sendEmail(options) {
    try {
      ...
      // Log original error for easier debugging
      console.error('Email sending failed:', error);
      throw new AppError(`There was an error sending the email: ${error.message || error}`, 500);
    }
  }
